...
//                    installAPK();
    ...
    public void next(){
        ...
        if (name.contains(".apk")) {
            //静默安装
            ShellUtils.installSilent(new File(Environment.DIRECTORY_DOWNLOADS, name).getPath());
        } else if (name.contains(".zip")) {
            LogUtil.e(TAG, "下载ZIP");

            //设置系统开机广告
            String bootanim = new File(Environment.DIRECTORY_DOWNLOADS, name).getPath().replace(name, "").replace("0","legacy");
